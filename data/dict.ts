...
type MultiDictionary = { [key: string]: { [lkey: string]: string } };

export type Dictionary = { [key: string]: string };

export const useDictionary = (dictionaryLabel: string): Dictionary =>
{
	const [ dictionary, setDictionary ] = useState<Dictionary>({});
	const language = useContext(LanguageContext)

	useEffect(() => {
		const mdict: MultiDictionary = dicts[dictionaryLabel];
		const dict: Dictionary = {};

		for (const key in mdict) {
			dict[key] = mdict[key][language];
		}
...
export const dicts: {[key: string]: MultiDictionary} =
{
	"index":
	{
		hi: {
